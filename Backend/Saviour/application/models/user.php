...
class User extends CI_Model{
	 ...
	 * params must contain 
	 */
	private function newEntry($params)//TODO enable user registration without facebook by 
	{
		$params['id']=$this->getLastId()+1;
		$this->id=$params['id'];//auto-increments id
		
		if($this->isParamsSet($params))
		{			
			foreach($this->getRequiredParams() as $p)
			{
				$this->$p=$params[$p];
			}
			
			$this->load->database();
			$this->db->insert('userlist', get_object_vars($this));
	...
	*/
	...
	{
		return array('id','name','fbid','email','pass','locationx','locationy','ready','fit','age','vehicle','slot','gender');
	}
		/**
